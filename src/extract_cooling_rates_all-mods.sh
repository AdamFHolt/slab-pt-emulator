...
for RUN_DIR in "$ROOT"/run_*; do
  echo "***********************************"
  MOD_NAME="$(basename "$RUN_DIR")"      # e.g., run_201
  ...
  if [[ -f "$INNER" ]]; then
    # --- Run extractor only if OUTDT doesn't exist ---
    if [[ ! -f "$OUTDT" ]]; then
      echo "[RUN ] $MOD_NAME (t1=$TIMESTEP1, t2=$TIMESTEP2, depths=$DEPTHS)"
      if ! "$EXTRACT" "$RUN_NUM" "$TIMESTEP1" "$TIMESTEP2" "$DEPTHS"; then
        echo "[FAIL] $MOD_NAME"
        NANS="NaN,NaN,NaN,NaN,NaN"
        for depth in $depth1 $depth2 $depth3; do
          case $depth in
            "$depth1") echo "$RUN_NUM,$NANS" >> "$MASTER1" ;;
            "$depth2") echo "$RUN_NUM,$NANS" >> "$MASTER2" ;;
            "$depth3") echo "$RUN_NUM,$NANS" >> "$MASTER3" ;;
          esac
        done
        continue
      fi
    ...
    # --- Append one row per requested depth (or NaNs if fields missing) ---
    for depth in $depth1 $depth2 $depth3; do
      line=$(awk -F, -v d="$depth" '
        NR>1 && ($1+0)==(d+0) {
          if ($3=="" || $5=="" || $6=="" || $7=="" || $8=="") print "NaN,NaN,NaN,NaN,NaN";
          else print $3,$5,$6,$7,$8;
          found=1; exit
        }
