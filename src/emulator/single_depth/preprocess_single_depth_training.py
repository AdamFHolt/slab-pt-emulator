...
SLABPT_ROOT_DEFAULT = THIS_FILE.parents[3]
...
def main():
    ...
    parser.add_argument("--suite", default="const-vc", help="Parameter suite name.")
    ...
    if args.depth_km is not None:
        if "depth_km" not in df_master.columns:
            raise ValueError("Asked for --depth-km but master has no 'depth_km' column.")
        df_master = df_master[np.isclose(df_master["depth_km"].astype(float), args.depth_km)].copy()
        df_master["obs_depth_km"] = float(args.depth_km)


    # Merge
    df = pd.merge(df_params, df_master, on="run_id", how="inner").drop_duplicates()
    print("Columns after merge:", sorted(df.columns))
    ...
    if args.suite != "const-vc":
        if all(c in df.columns for c in ["v_conv", "t_conv"]):
            df["v_conv_over_tconv"] = df["v_conv"] / df["t_conv"].replace(0, np.nan)
            feat_cols.append("v_conv_over_tconv")
            # extra_feats.append("vConvOverTconv")
    print("Picked feat_cols:", feat_cols)
