...
sp_age = float(sys.argv[3]) # [Ma]
op_age = float(sys.argv[4]) # [Ma]

ofile_t = f"outputs/temp_crust-dip{dip_crust}_crust-depth{depth_full_crust}_sp-age{sp_age}_op-age{op_age}.txt"
ofile_c = f"outputs/comp_crust-dip{dip_crust}_crust-depth{depth_full_crust}_sp-age{sp_age}_op-age{op_age}.txt"
oplot   = f"outputs/crust-dip{dip_crust}_crust-depth{depth_full_crust}_sp-age{sp_age}_op-age{op_age}.png"
...
plt.subplot(3, 1, 3)
levels = np.linspace(Tmin, 1700, 41)  
sz3 = plt.tricontourf(T[:,0]/1e3, T[:,1]/1e3, T[:,2], levels=levels, cmap='coolwarm')
