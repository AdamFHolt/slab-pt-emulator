...
ofile_t = f"outputs/temp_crust-dip{dip_crust}_sp-age{sp_age}_op-age{op_age}_plate-thick{plate_thick}_crust-thick{y_crust}_free-plates.txt"
ofile_c = f"outputs/comp_crust-dip{dip_crust}_sp-age{sp_age}_op-age{op_age}_plate-thick{plate_thick}_crust-thick{y_crust}_free-plates.txt"
oplot   = f"outputs/crust-dip{dip_crust}_sp-age{sp_age}_op-age{op_age}_plate-thick{plate_thick}_crust-thick{y_crust}_free-plates.png"
...
gap_length = 500e3 			# [m] length of gap between plates and boundaries
...
for j in range(ynum + 1): 
	for i in range(xnum + 1):
		...
		# flat portion of SP
		if x <= (gap_length + 500.e3) and x > gap_length and y > (ymax - plate_thick):
				T_term2 = 0
				ridge_age = ((x - gap_length) / (500.e3)) * sp_age
				for n in range(1,8):
		...
		elif x <= (x_SP-radius_outer) and x > (gap_length + 500.e3) and y > (ymax - plate_thick):
				T_term2 = 0
				for n in range(1,8):
		...
		elif x >= (x_SP + (300e3)/np.tan(np.radians(dip_crust))) and x < (xmax - gap_length - 500e3) and y > (ymax - plate_thick):
		...
		elif x >= (xmax - gap_length - 500e3) and x < (xmax - gap_length) and y > (ymax - plate_thick):

			T_term2 = 0
			ridge_age = (((xmax - gap_length)-x)/500e3) * op_age
			for n in range(1,8):
				T_term2 += ((2*Tmax)/(n*np.pi))*np.sin(n*np.pi*(ymax-y)/plate_thick)*np.exp((-1.* n**2 * (np.pi)**2 * k * ridge_age * Ma_to_sec)/(plate_thick**2))
			T[ind,2]='%.5f'  %   (T_term1 + T_term2)
		...
		if x >= (gap_length+250e3) and x <= (x_SP-radius_outer) and y > (ymax - y_crust):
			C[ind,2]=1
		...
		elif x >= (x_SP + (300e3)/np.tan(np.radians(dip_crust))) and x <= (xmax - gap_length):
