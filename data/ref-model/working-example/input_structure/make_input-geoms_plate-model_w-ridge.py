...
ofile_t = f"outputs/temp_crust-dip{dip_crust}_sp-age{sp_age}_op-age{op_age}_plate-thick{plate_thick}_crust-thick{y_crust}_w-ridge.txt"
ofile_c = f"outputs/comp_crust-dip{dip_crust}_sp-age{sp_age}_op-age{op_age}_plate-thick{plate_thick}_crust-thick{y_crust}_w-ridge.txt"
oplot   = f"outputs/crust-dip{dip_crust}_sp-age{sp_age}_op-age{op_age}_plate-thick{plate_thick}_crust-thick{y_crust}_w-ridge.png"
...
for j in range(ynum + 1): 
	for i in range(xnum + 1):
		...
		if x <= (1500.e3) and y > (ymax - plate_thick):
				T_term2 = 0
				ridge_age = 2 + (x/1500.e3) * (sp_age-2)
				for n in range(1,8):
		...
		if x >= 1000.e3 and x <= (x_SP-radius_outer) and y > (ymax - y_crust):
			C[ind,2]=1
		...
		elif x >= (x_SP + (300e3)/np.tan(np.radians(dip_crust))):
